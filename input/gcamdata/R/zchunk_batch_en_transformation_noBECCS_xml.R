...
#' module_energy_batch_en_transformation_noBECCS_xml
#'
#' Construct XML data structure for \code{en_transformation_noBECCS.xml}.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{en_transformation_noBECCS.xml}. The corresponding file in the
#' original data system was \code{batch_en_transformation_noBECCS.xml.R} (energy XML).
module_energy_batch_en_transformation_noBECCS_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c(XML = "en_transformation_noBECCS.xml"))
    ...
    L222.GlobalTechShrwt_en <- get_data(all_data, "L222.GlobalTechShrwt_en") # MODIFY
    ...
    # Shut down BECCS in refining
    L222.GlobalTechShrwt_en %>%
      filter(subsector.name != "biomass liquids") -> other_refining

    L222.GlobalTechShrwt_en %>%
      filter(subsector.name == "biomass liquids",
             !grepl("CCS", technology)) -> other_biomass_liquids

    L222.GlobalTechShrwt_en %>%
      filter(subsector.name == "biomass liquids",
             grepl("CCS", technology)) %>%
      mutate(share.weight = 0) %>%
      bind_rows(other_biomass_liquids, other_refining)-> L222.GlobalTechShrwt_en_noBECCS

    # Produce outputs
    create_xml("en_transformation_noBECCS.xml") %>%
      ...
      add_xml_data(L222.GlobalTechShrwt_en_noBECCS, "GlobalTechShrwt") %>%
      ...
      add_precursors("L222.Supplysector_en",
                     ...
                     "L222.StubTechProd_refining",
                     "L222.StubTechCoef_refining") ->
      en_transformation_noBECCS.xml

    return_data(en_transformation_noBECCS.xml)
  } else {
